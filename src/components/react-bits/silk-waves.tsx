...
const SilkWaves: React.FC<SilkWavesProps> = ({
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      u.uResolution.value.set(newWidth, newHeight);
    };
  ...
  useEffect(() => {
    ...
    u.uC1.value.set(colors[0] ?? "#000");
    u.uC2.value.set(colors[1] ?? "#000");
    u.uC3.value.set(colors[2] ?? "#000");
    u.uC4.value.set(colors[3] ?? "#000");
    u.uC5.value.set(colors[4] ?? "#000");
    u.uC6.value.set(colors[5] ?? "#000");
    u.uC7.value.set(colors[6] ?? "#000");
    u.uC8.value.set(colors[7] ?? "#000");
  }, [
    speed,
    scale,
    distortion,
