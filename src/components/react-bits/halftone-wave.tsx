...
function parseHexColor(hex: string): [number, number, number] {
  ...
  return [
    parseInt(match[1] ?? "0", 16) / 255,
    parseInt(match[2] ?? "0", 16) / 255,
    parseInt(match[3] ?? "0", 16) / 255,
  ];
}
...
const WaveScene: React.FC<WaveSceneProps> = ({
  ...
  useFrame((state, delta) => {
    ...
    const u = mat.uniforms as {
      ...
      uContrastMin: { value: number };
      uContrastMax: { value: number };
      uScrollX: { value: number };
    ...
    u.uGridDensity.value = gridDensity;
    u.uDotSize.value = dotSize;
    u.uSoftness.value = softness;
    u.uContrastMin.value = contrastMin;
    u.uContrastMax.value = contrastMax;
    u.uScrollX.value = scrollX;
    u.uScrollY.value = scrollY;
    u.uRotation.value = (rotation * Math.PI) / 180;
    u.uColorA.value.set(...colorARgb);
    u.uColorB.value.set(...colorBRgb);
