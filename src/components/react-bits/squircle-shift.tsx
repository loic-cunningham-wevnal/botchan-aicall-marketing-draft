...
const ShaderPlane: React.FC<ShaderPlaneProps> = ({
  ...
  useFrame((state) => {
    ...
    const u = materialRef.current.uniforms as {
      ...
      u_colorLayers: { value: number };
      u_gridFrequency: { value: number };
      u_gridIntensity: { value: number };
      u_waveSpeed: { value: number };
      u_waveIntensity: { value: number };
      u_spiralIntensity: { value: number };
      u_lineThickness: { value: number };
      u_falloff: { value: number };
      u_centerX: { value: number };
      u_centerY: { value: number };
      u_colorTint: { value: THREE.Color };
      u_backgroundColor: { value: THREE.Color };
      u_brightness: { value: number };
      u_phaseOffset: { value: number };
    };
    u.u_time.value = state.clock.elapsedTime;
    u.u_resolution.value.set(viewport.width * 100, viewport.height * 100);
    u.u_speed.value = speed;
    u.u_colorLayers.value = colorLayers;
    u.u_gridFrequency.value = gridFrequency;
    u.u_gridIntensity.value = gridIntensity;
    u.u_waveSpeed.value = waveSpeed;
    u.u_waveIntensity.value = waveIntensity;
    u.u_spiralIntensity.value = spiralIntensity;
    u.u_lineThickness.value = lineThickness;
    u.u_falloff.value = falloff;
    u.u_centerX.value = centerX;
    u.u_centerY.value = centerY;
    u.u_colorTint.value.set(colorTint);
    u.u_backgroundColor.value.set(backgroundColor);
